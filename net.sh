...
EOF
	...
	install -d -m 0755 /etc/apt/keyrings;wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- |tee /etc/apt/keyrings/packages.mozilla.org.asc > /dev/null;gpg -n -q --import --import-options import-show /etc/apt/keyrings/packages.mozilla.org.asc | awk '/pub/{getline; gsub(/^ +| +$/,""); if($0 == "35BAA0B33E9EB396F59CA838C0BA5CE6DC6315A3") print "\nThe key fingerprint matches ("$0").\n"; else print "\nVerification failed: the fingerprint ("$0") does not match the expected one.\n"}';echo "deb [signed-by=/etc/apt/keyrings/packages.mozilla.org.asc] https://packages.mozilla.org/apt mozilla main" | tee -a /etc/apt/sources.list.d/mozilla.list > /dev/null;echo -e 'Package: *\nPin: origin packages.mozilla.org\nPin-Priority: 1000' |tee /etc/apt/preferences.d/mozilla;apt-get update && apt-get -y install gcc make xfce4 xfce4-goodies tightvncserver firefox firefox-l10n-zh-cn;echo 'export USER=root' >> /root/.bashrc;source /root/.bashrc;apt-get -y install fonts-wqy-microhei fonts-wqy-zenhei xfonts-wqy
	curl -Lk https://github.com/lubxun/test/raw/refs/heads/main/ssr.sh|/bin/bash
fi
rm -f /root/stop &>/dev/null
t=$(tail -100 nohup.out|grep COLAB |wc -l)
if [ "$t" -gt 15 ]
	then touch /root/stop
fi
if [ ! -f /root/h.pac ]
	then /usr/sbin/iptables -P INPUT ACCEPT
	curl -LkO https://github.com/lubxun/test/raw/refs/heads/main/a.pac
	...
	nohup python -m http.server 19203 -d /root/ &
